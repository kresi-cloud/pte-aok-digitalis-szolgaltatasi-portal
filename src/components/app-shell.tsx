...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const hasPendingApproval = useMemo(
    () =>
      store.requests.some((r) =>
        r.approvals.some((a) => a.decision === "fuggoben" && a.approverId === store.currentUser.id),
      ),
  ...
  const items = useMemo(
    () =>
      NAV.filter(
        (n) =>
          n.roles.includes(store.activeRole) || (n.to === "/jovahagyasok" && hasPendingApproval),
      ),
                  ...
                  {store.users
                    .filter(
                      (u) =>
                        ...
                        (store.extraUsers ?? []).some((e) => e.id === u.id),
                    )
                    .map((u) => (
